...
MDEFVAR_OPTDENSE_float(fDepthDiffThreshold, "Depth Diff Threshold", "maximum variance allowed for the depths during refinement", "0.03")
...
float DepthEstimator::ScorePixelImage(const ViewData& image1, Depth depth, const Normal& normal)
{
	...
	for (const NeighborEstimate& neighbor: neighborsClose) {
        ...
        ASSERT(ISFINITE(factorDepth));
        ASSERT(ISFINITE(factorNormal));
    }
	#endif
...
bool MVS::ExportOverlayedImage(const String& fileName, const Image8U3& im, const DepthMap& depthMap, Depth minDepth, Depth maxDepth, bool sparse)
{

    VERBOSE("Entering ExportOverlayed");

    auto t = im.getData();
    auto w = im.size().width;
    auto h = im.size().height;
...
//                VERBOSE("x = %u, y = %u  z = %f ", x,y,depthMap(y,x));
    ...
    cv::applyColorMap(depthMapCv, colorizedDepthMap,2);
    for(auto x=0;x<w;++x){
        for(auto y=0;y<h;++y){
            if(depthMap(y,x) < eps)
                colorizedDepthMap.at<cv::Vec3b>(y,x) = cv::Vec3b(0,0,0);
            else{
                if (sparse){
                    auto curCol = colorizedDepthMap.at<cv::Vec3b>(y,x);
                    colorizedDepthMap.at<cv::Vec3b>(y,x) = cv::Vec3b(0,0,0);
                    cv::circle(colorizedDepthMap, cv::Point(x,y), 5,curCol,-1);
                }
            }
        }
    }
    ...
    cv::addWeighted(im, .35, colorizedDepthMap, .65, 0, dst);
    colorizedDepthMap.release();
