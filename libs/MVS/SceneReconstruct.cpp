...
struct vert_info_t {
    ...
    uint8_t sensor; //True if comes from lidar measurement
    float alphaVisLidar; //#points in the same voxel as the centroid
	...
	inline vert_info_t() {}
	#endif
    void InsertSensor(PointCloud::Sensor s){
       sensor = s;
    }
    void UpdateLidarVis(float vis){
        alphaVisLidar = vis;
    }
...
void Scene::transformPCL2MVS(const std::string& filename){
        ...
        if (useLidar){
            VERBOSE("Using LiDAR for mesh reconstruction!");
            pcl::PointCloud<pcl::XPointXYZ> pFiltered;
            int success = pcl::io::loadPCDFile<pcl::XPointXYZ>(filename, pFiltered);
            ASSERT(success!=-1);
        ...
        pointCloudLidarCam.pointViews = pViews;
...
)
{
...

	{
        ...
        std::vector<point_t> vertices(pointCloudLidarCam.points.GetSize());
        std::vector<std::ptrdiff_t> indices(pointCloudLidarCam.points.GetSize());
		// fetch points

        FOREACH(i, pointCloudLidarCam.points) {
            const PointCloud::Point& X(pointCloudLidarCam.points[i]);
		...
		std::for_each(indices.cbegin(), indices.cend(), [&](size_t idx) {
            ...
            if (sensor==LIDAR_WEIGHT){
               views = defaultLidarViewsArray;
            }
            float nClustered = pointCloudLidarCam.visibilities[idx];
            if (true){
			...
			if (hint == vertex_handle_t()) {
                ...
                hint->info().InsertSensor(sensor);
                if (sensor == LIDAR_WEIGHT)
                    hint->info().UpdateLidarVis(nClustered);
                ASSERT(hint != vertex_handle_t());
			} else
			if (distInsert <= 0) {
                // insert all points Code Code Code Code
...
//                std::cerr << "Found 1 infinite cell!" << std::endl;
	...
	// add alpha_vis(point) to cell's directed edge in the graph
	{
		...
		for (delaunay_t::Vertex_iterator vi=delaunay.vertices_begin(), vie=delaunay.vertices_end(); vi!=vie; ++vi) {
			...
			FOREACH(v, vert.views) {
                ...
                const edge_cap_t alpha_vis = (vertSensor == LIDAR_WEIGHT) ? vert.alphaVisLidar : view.weight;
				...
				do {
                    ...
                    const edge_cap_t distanceBonusCamera = (vertSensor == LIDAR_WEIGHT) ? 1 : (1.f-EXP(-SQUARE((float)inter.dist)*inv2SigmaSq));
